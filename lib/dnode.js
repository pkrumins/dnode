...
function DNode (wrapper) {
    ...
    */
    ...
    self.withStream = function (stream, opts, block) {
       ...
       conn.on('localError',function(err){ //required behaviour for test/asynct/error.asynct.js
               self.emit('localError',err);
       });	 	
       conn.on('remoteError',function(err){ //required behaviour for test/asynct/error.asynct.js
